...
class Encoder(nn.Module):
    def __init__(
            ...
            use_texture_conv=True, use_motion_conv=False, texture_downsample=True,
...
class Encoder_No_texture(nn.Module):
    def __init__(
            self, dim_in=3, dim_out=32, num_resblk=3,
            use_texture_conv=True, use_motion_conv=True, num_resblk_motion=2
    ):
        super(Encoder_No_texture, self).__init__()
        self.use_texture_conv, self.use_motion_conv = use_texture_conv, use_motion_conv
...
class Decoder(nn.Module):
    def __init__(self, dim_in=32, dim_out=3, num_resblk=9, texture_downsample=True):
        ...
        if self.texture_downsample:
            self.texture_up = Conv2D_activa(dim_in, 32, 3, 1, 1, activation='relu')
...
class Manipulator(nn.Module):
    def __init__(self):
        ...
        self.g = Conv2D_activa(32, 32, 7, 1, 3, activation='relu')
